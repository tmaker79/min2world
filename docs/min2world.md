...
Milestone 16에서는 생산 특화 시설의 지원 관계와 정착 거점별 생산 슬롯을 추가한다. Farm·Mine·Blacksmith는 가장 가까운 아군 Town·City에서 통행 가능한 육지 그래프 거리 3 이내에 있어야 하며, Village는 0개, Town은 2개, City는 4개까지 생산 거점을 지원한다. 같은 거리에서는 정착 거점 ID 오름차순으로 귀속하며 Farm·Mine·Blacksmith 본체만 슬롯 1개를 사용하고 농지와 군사 거점은 사용하지 않는다. 지원 관계는 저장하지 않고 현재 거점 상태에서 계산한다. 점령·발전으로 한도를 초과해도 기존 거점은 유지하며 한도 이하가 될 때까지 새 생산 거점 건설만 막는다.
