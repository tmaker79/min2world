...
- 2인용 15×11 지도는 화면 중앙 열을 강으로 고정하고 위에서 네 번째·여덟 번째 행에 대칭인 평지 통로를 둔다. 통로와 좌우 접근 타일은 거점·시작 유닛 배치에서 예약한다.
...
type GameState = {
  ...
  mapSeed: string
  mapType: MapType
  mapGenerationVersion: number // 23
  boardSize: BoardSize
...
- 후속 맵 생성 버전 23에서 15×11 2인용 지도에 중앙 강과 두 평지 통로를 추가
...
- 마이그레이션은 저장된 `mapGenerationVersion`을 바꾸지 않으며, 새 지도 생성 버전은 23을 사용한다.
- 맵 생성 버전 5부터 현재 버전 23까지 저장된 타일을 재생성하지 않고 지원한다.
