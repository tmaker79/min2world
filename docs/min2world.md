...
Farm·Mine·Blacksmith는 자기 영토에만 건설할 수 있다. 분쟁·적·미편입 지역에는 건설할 수 없으며 기존 지형, 거점 간격, 아군 상위 거점에서 육로 거리 3 이내 조건도 함께 적용한다. Village와 Outpost 건설에는 영토 제한을 적용하지 않는다. 건설자 거점과 민간 유닛의 누적·동시 보유 상한은 없으며, 영토는 현재 생산 거점 건설과 지도 표시에만 사용하고 이동·전투·수입에는 보정을 주지 않는다.
