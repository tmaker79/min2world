...
1. 2인용 빠른대전을 1인 개발 범위 안에서 작지만 완결된 전략 게임으로 다듬는다.
2. AI와 협업하는 소프트웨어 개발 방법과 검증 과정을 탐구한다.
3. 게임을 만들면서 React와 TypeScript를 실전 수준으로 익힌다.
4. 컴포넌트 설계, 상태 관리, 비즈니스 로직 분리, 테스트 경험을 `wizard-web` 개발에 활용한다.
...
현재 공개 메인은 핵심 전투·생산·점령 규칙에 집중한 **2인용 빠른대전**이다. 새로운 시스템을 추가하기보다 빠른대전의 게임성·사용성·안정성을 먼저 높이며, 확장 규칙은 전체모드에서 보존한다. 빠른대전의 완성도를 충분히 확보한 뒤 보류한 기능 확장 마일스톤을 재개한다.

프로젝트 소개와 빠른대전 조작은 [루트 README](../README.md), 게임 모드와 규칙은 [게임 가이드](game-guide.md), 단계별 기록은 [개발 마일스톤](milestones/README.md)을 따른다. 완료된 마일스톤 문서는 덮어쓰지 않는다.
...
1. 2세력·15×11 균형 지도의 새 게임을 즉시 시작한다.
2. 플레이어 유닛을 이동하고 적 부대와 수도를 공격한다.
3. 중립 생산 거점을 점령해 수입을 늘리고 도시에서 군사 유닛을 생산한다.
4. 턴을 종료하면 수입과 유지비를 정산하고 AI가 행동한다.
5. 상대 수도를 점령하면 승리하고 내 수도를 잃으면 패배한다.

빠른대전에서는 거점 발전·도시 건설·정착·생산 거점 건설과 민간 유닛 생산을 사용하지 않는다. 이 기능은 전체모드에서 유지한다.
...
- axial 좌표의 뾰족형 육각 지도
- `min2world.dev`에서는 2인용 빠른대전을 즉시 시작하고, `beta.min2world.dev`에서는 전체모드를 제공한다.
- 빠른대전은 한국어·영어 UI를 지원하며 거점 발전·건설·민간 유닛 생산을 제외한다.
- 전체모드 시작 화면에서 지도 크기·지도 종류·내 세력을 선택한다.
...
소유된 군사 거점은 전체 footprint의 인접 칸에 통제 구역을 만들지만 중립 거점은 만들지 않는다. 적 또는 중립 군사 거점에는 공성 공격을 사용하며 거점은 반격하지 않는다. 자기 세력의 요새화 거점(군사 거점·City) 위에 주둔한 유닛은 직접 공격할 수 없으며, 공격 세력은 거점을 먼저 공성해 점령해야 한다. HP가 0이 되면 즉시 공격 세력이 점령하고 최대 HP의 50%를 회복한다. 이동 점령과 공성 점령 모두 점령 거점의 `lastDevelopedTurn`을 현재 턴으로 설정해 점령한 턴에는 발전할 수 없다. `Outpost → Keep → Stronghold` 발전은 기존 HP 비율을 유지하고, `Town → City`는 최대 HP로 시작한다.

Outpost·Keep·Stronghold는 요충지 방어 시설로서 수입·영토·유닛 생산 능력이 없다. 발전은 최대 HP와 방어력을 높이며, 모든 유닛 생산은 City에서 담당한다.

### 구현된 영토 구조

소유된 정착지는 발전 단계에 따라 주변 타일을 영토로 편입한다. `Village`는 육각 거리 1, `Town`은 거리 2, `City`는 거리 3까지 영향을 준다. 군사 거점·생산 특화 시설·중립 거점은 영토를 만들지 않는다.
...
Farm·Mine·Blacksmith는 자기 영토에만 건설할 수 있다. 분쟁·적·미편입 지역에는 건설할 수 없으며 기존 지형, 거점 간격, 아군 상위 거점에서 육로 거리 3 이내 조건도 함께 적용한다. 거점 간격(육각 거리 2)은 영토가 없는 군사 거점을 제외하므로 군사 거점 바로 옆에도 생산 거점을 건설할 수 있다. Outpost는 일반 평지·언덕·숲 중 적 영토가 아닌 타일에 건설하며 기존 군사 거점과 인접할 수 없다. 정착·생산 거점과의 거리 및 아군 상위 거점 연결 거리 제한은 Outpost에 적용하지 않는다. 건설자 거점과 민간 유닛의 누적·동시 보유 상한은 없으며, 영토는 현재 생산 거점 건설과 지도 표시에만 사용하고 이동·전투·수입에는 보정을 주지 않는다.

Milestone 16의 첫 단계로 생산 특화 시설의 지원 관계와 정착지별 기본 생산 거점 한도를 구현했다. 건설자가 세운 Farm·Mine·Blacksmith는 가장 가까운 아군 Town·City에서 통행 가능한 육지 그래프 거리 3 이내에 있어야 하며, Village는 0개, Town은 2개, City는 4개까지 생산 거점을 지원한다. 같은 거리에서는 정착 거점 ID 오름차순으로 귀속하며 건설자가 세운 Farm·Mine·Blacksmith 본체만 한도에 포함한다. 맵에서 생성된 중립 생산 거점은 점령 후에도 한도에서 제외하고 군사 거점도 포함하지 않는다. 지원 관계는 저장하지 않고 현재 거점 상태에서 계산한다. 점령·발전으로 한도를 초과해도 기존 생산 거점은 유지하며 한도 이하가 될 때까지 새 생산 거점 건설만 막는다. City 외곽 지구를 완공해 한도를 최대 6개로 늘리는 기능은 Milestone 16의 남은 범위다.
...
- 게임성: 한 판의 핵심 전투·생산·점령 경험을 다듬는다.
- 사용성: 조작과 정보 전달을 명확하게 하고 데스크톱·모바일 기본 호환을 유지한다.
- 안정성: 게임 규칙, AI, 저장, UI의 회귀를 테스트하고 배포 빌드의 신뢰성을 유지한다.

Milestone 16의 남은 거점 확장과 Milestone 17의 도로 건설을 포함한 신규 시스템은 빠른대전의 완성도를 충분히 확보한 뒤 재개한다. 기능 확장을 재개할 때는 해당 마일스톤 명세와 당시 저장 스키마를 기준으로 구현한다.

현재 기준 시나리오는 다음과 같다.

> 2세력·15×11 무작위 지도에서 부대를 이동·공격하고 중립 생산 거점을 점령한다. 도시에서 군사 유닛을 생산하며 AI와 턴을 주고받고, 상대 수도를 점령하면 한 판이 끝난다.

변경할 때는 이 시나리오의 규칙·AI·UI 테스트가 계속 통과하는지 확인한다.
