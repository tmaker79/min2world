...
시작 화면에서 맵 크기·지도 종류·내 세력을 고른 뒤, 무작위 육각 지도에서 부대를 이동하고 전투하며 상대 수도를 점령해 승리합니다. 도시에서 보병·기병·궁병·창병을 생산하고 내정 건물을 건설할 수 있으며, AI도 같은 경제·생산·건설·전투 규칙으로 행동합니다. 플레이어 턴은 브라우저에 저장해 이어서 플레이할 수 있습니다.
...
7. 거점을 좌클릭하면 거점 정보창이 열립니다. 소유 거점의 `발전` 탭에서 비용과 다음 효과를 확인해 발전할 수 있으며, Village와 Town은 지도에 표시된 footprint 방향을 먼저 선택합니다. 생산 가능한 거점에는 `생산` 탭이 표시되고, City의 `건설` 탭에서는 곡창·시장·성벽·병영·선술집·신전·도서관을 건설하거나 진행 중인 건설을 취소할 수 있습니다. 도시당 건설 대기열은 하나이며 취소해도 자원은 환불되지 않습니다.
...
| Village / Town / City | 3 / 5 / 7 | City에서 가능 |
...
- 현재 스키마는 12입니다. 스키마 6의 세력 ID부터 스키마 11의 건물 없는 거점까지 연쇄 마이그레이션해 불러옵니다. 스키마 8 저장에 `mapType`이 없으면 `balanced`로 처리하고, 스키마 9 군사 거점은 종류별 최대 HP로 채웁니다. 마이그레이션 중 기존 맵 생성 버전은 유지하며 새 지도는 버전 24를 사용하고 버전 23 이하 저장도 계속 지원합니다. 사각 지도 기반 스키마 4·5는 불러오지 않습니다.
