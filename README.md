...
시작 화면에서 맵 크기·지도 종류·내 세력을 고른 뒤, 무작위 육각 지도에서 부대를 이동하고 전투하며 상대 수도를 점령해 승리합니다. 도시에서 보병·기병·궁병·창병을 생산하고 내정 건물을 건설할 수 있으며, 병력 유지비와 다음 정산 예약액을 고려해 경제를 운영합니다. AI도 같은 경제·생산·건설·전투 규칙으로 행동합니다. 플레이어 턴은 브라우저에 저장해 이어서 플레이할 수 있습니다.
...
8. 유닛을 선택하면 성 정보창과 같은 스타일의 부대 정보창이 열립니다. 활성 플레이어 턴에는 `해산`으로 부대를 즉시 제거할 수 있으며 자원은 환불되지 않습니다.
9. 상태바에서 자원·수입·유지비·순수입을 확인합니다. 플레이어 유닛의 행동을 마치면 `턴 종료`를 선택하거나 `Enter`를 누릅니다. 생산 배치 중에는 `Esc`로 취소합니다.
